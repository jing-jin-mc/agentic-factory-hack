...
{
    /// <summary>
    /// Get the required skills for repairing a specific fault type
    /// </summary>
    IReadOnlyList<string> GetRequiredSkills(string faultType);

    /// <summary>
    /// Get the required part numbers for repairing a specific fault type
    /// </summary>
...
/// In a production system, these mappings would typically come from a knowledge base or expert system.
/// </summary>
public sealed class FaultMappingService : IFaultMappingService
{
    // Hardcoded mappings: fault type -> required skills
    ...
    // Hardcoded mappings: fault type -> required part numbers
    private static readonly IReadOnlyDictionary<string, IReadOnlyList<string>> FaultToParts =
        new ReadOnlyDictionary<string, IReadOnlyList<string>>(
            new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["high_radial_force_variation"] = Array.Empty<string>(), // No parts needed
    ...
    // Default fallback for unknown fault types
